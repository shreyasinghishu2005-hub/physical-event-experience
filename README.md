...
If your Render service still uses an older build command such as `npm install`, the backend `postinstall` hook now builds the frontend during Render's install phase before the production server starts.
